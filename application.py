...
from boto3.dynamodb2.exceptions import ConditionalCheckFailedException
...
ddb_table = ddb_client.Table(table_name=application.config['STARTUP_SIGNUP_TABLE'])
...
def store_in_dynamo(signup_data):
    ddb_client.put_item(TableName=ddb_table, Item={signup_data})
...
def publish_to_sns(signup_data):
    try:
        sns_client.publish(TopicArn = application.config['NEW_SIGNUP_TOPIC'], Message = f'{json.dumps(signup_data)}, "New signup: {signup_data['email']}')
